...
page_fault_handler(struct Trapframe *tf)
{
	...
	//cprintf("utf->fault_va: %x", utf->utf_fault_va);
...
static uint32_t tmp_eip = 0;
static int tmpbrk_index = 0;

static void pre_breakpoints(struct Trapframe *tf){
	if(tmp_eip != 0){// when triggering Debug interrupt following the breakpoint.
		*((uint8_t *)tmp_eip) = 0xcc;
		tf->tf_eflags = breakpoints[tmpbrk_index].eflags;
		tmp_eip = 0;
		tmpbrk_index = 0;
	}

	int brk_index = eip2brk(tf->tf_eip - 1);
	...
	uint32_t eip = breakpoints[brk_index].eip;
	tmp_eip = eip;
	uint8_t op = breakpoints[brk_index].op;
	*((uint8_t *)eip) = op;
	breakpoints[brk_index].eflags = tf->tf_eflags;
	
	tf->tf_eflags |= 0x100;
	tmpbrk_index = brk_index;
	return;
}


static void post_breakpoints(struct Trapframe *tf){
	if(tmp_eip != 0){
		tf->tf_eflags |= 0x100;
	}
}
